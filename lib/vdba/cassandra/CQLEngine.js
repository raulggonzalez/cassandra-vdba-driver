 ...
 * @param {Function} callback The function to call: fn(error, result).
 */
CQLEngine.prototype.findp = function findp(cql, params, callback) {
  this.connection.native.execute(cql, params, function(error, result) {
    if (error) callback(error);
    else callback(undefined, new CassandraResult(result.rows));
  });
};
 ...
 * @param {Function} callback The function to call: fn(error, row).
 */
CQLEngine.prototype.findOnep = function findOnep(cql, params, callback)  {
  var one = false;
  ...
  this.connection.native.eachRow(cql, params, process, function(error) {
    if (error && !one) {
      callback(error);
    }
  });
